...
def aggregation_namedtuple(
    *args,
    ...
    option_map = {"cohort": "cohort", "time": "time", "event": "relative_period"}
    if stratum is not None:
...
def get_agg_attr_name(type_of_aggregation: str = None, overall: bool = False) -> str:
...
def filter_gt_dict(group_time: list[dict], filter_gt: dict) -> list[dict]:
    """
...
def varying_base_period(
    cohort_ar: np.ndarray,
    time_ar: np.ndarray,
    anticipation: int,
) -> list[dict]:
    """
    ...
    for cohort in cohort_ar:
        for t_idx, _time in enumerate(time_ar[:-1]):
...
def universal_base_period(
    ...
    cbt = []
    for cohort in cohort_ar:
        for t_idx, _time in enumerate(time_ar):
            ...
            cbt.append({"cohort": c, "base_period": b, "time": t})
...
def stack_influence_funcs(ntl: list[namedtuple], return_idx: bool = False):
    ...
    if influence_func_is_sparse:
        inf_funcs = hstack(
            [r.influence_func for r in ntl if r.influence_func is not None]
        )
        inf_funcs = inf_funcs.toarray()  # faster mboot if dense matrix
    else:
        inf_funcs = np.stack(
            [r.influence_func for r in ntl if r.influence_func is not None], axis=1
        )

    if return_idx:
        # indexes for the non-missing influence_func
        not_nan_idx = np.array(
            [i for i, r in enumerate(ntl) if r.influence_func is not None]
        )
...
def get_idx_of_values(main_list: list | Index, list_of_values: list):
    return [idx for idx, v in enumerate(main_list) if v in list_of_values]


def _idx_to_arr(idx_array, length):
    arr = np.zeros(length)
    arr[idx_array] = 1
    return arr
...
def wald_pre_test(ntl: list[namedtuple]):
    ...
    pre_ntl = filter_ntl(ntl=ntl, pre=True, non_zero_influence_func=True)
    ...
    pre_vcv, n = get_vcv_from_if(
        inf_funcs=stack_influence_funcs(pre_ntl), return_n_obs=True
    )
    ...
    return {"W": round(w, 6), "p_value": round(w_p_value, 6)}
...
def preprocess_est_method(est_method) -> tuple:
    ...
    if est_method not in possible_est_methods:
        raise ValueError(
            f"Invalid est_method. 'est_method' must be one of {possible_est_methods}"
        )
...
def preprocess_base_delta(
    ...
    if isinstance(base_delta, str) or isinstance(base_delta, list):
        if "base" in base_delta and "delta" in base_delta:
            ...
            dropped_cols = [i for i in x_covariates if i not in time_varying_x]
            if dropped_cols:
                warn(
                    f"if base_delta = delta only time varying covariates will be used: "
        ...
        if x_base and x_delta is None:
            ...
            if non_tv_x_delta:
                warn(
                    f'when specifying base_delta the list for "delta" was restricted and '
        ...
        if missing_cols:
            warn(
                f"when specifying base_delta the analysis was restricted "
...
def preprocess_fit_cluster_arg(
    cluster_var: list | str, entity_name: str, true_rc: bool
) -> tuple[str | None, bool]:
    ...
    if cluster_var is not None:
        ...
        if (
            not cluster_var
        ):  # don't cluster by entity if empty list or any falsy (but None)
            ...
            if cluster_var:  # there is an extra cluster_var
                if len(cluster_var) > 1:
                    raise ValueError(
                        f"only 2 cluster_var allowed where one is {entity_name}"
                    )
...
def preprocess_aggregate_cluster_arg(
    cluster_var: list | str, entity_name: str, cluster_by_entity: bool
) -> str | None:
    ...
    if cluster_var is not None:
        ...
        if entity_name in cluster_var:
            if not cluster_by_entity:
                ...
                raise ValueError(
                    "cluster_var: should cluster by entity in .fit() "
...
def _extract_dict_strata_ntl(
    result_dict: dict, attr_name: str, strata: list, items: bool = False
):
    if attr_name == "att_gt":

        out_dict = {
            g: [nt for nt in result_dict["full_sample"]["ATTgt_ntl"] if nt.stratum == g]
            for g in strata
        }
        ...
        out_dict = {
            g: [
                nt
                for nt in getattr(
                    result_dict["full_sample"]["aggregate_inst"], attr_name
                )
                if nt.stratum == g
            ]
            for g in strata
...
def _extract_dict_samples_ntl(
    result_dict: dict, attr_name: str, sample_names: list, items: bool = False
):
    if attr_name == "att_gt":

        out_dict = {s: result_dict[s]["ATTgt_ntl"] for s in sample_names}
...
def _extract_dict_sg_ntl(
    ...
    if dict_keys == "s":
        ...
        if attr_name == "att_gt":
            ...
            out_dict = {
                s: {
                    g: [nt for nt in result_dict[s]["ATTgt_ntl"] if nt.stratum == g]
                    for g in strata
                }
                for s in sample_names
            }
            ...
            out_dict = {
                s: {
                    g: [
                        nt
                        for nt in getattr(result_dict[s]["aggregate_inst"], attr_name)
                        if nt.stratum == g
                    ]
                    for g in strata
                }
        ...
        if attr_name == "att_gt":
            out_dict = {
                g: {
                    s: [nt for nt in result_dict[s]["ATTgt_ntl"] if nt.stratum == g]
                    for s in sample_names
                }
                for g in strata
            }
        ...
        if attr_name == "att_gt":
            ...
            out_dict = {
                (s, g): [nt for nt in result_dict[s]["ATTgt_ntl"] if nt.stratum == g]
                for g in strata
                for s in sample_names
            }
        else:
...
def extract_dict_ntl(
    ...
    if sample_names and strata:
        ...
        out_dict = _extract_dict_strata_ntl(
            result_dict=result_dict, attr_name=attr_name, strata=strata, items=items
        )
        ...
        if attr_name == "att_gt":
            out_dict = {"full_sample": result_dict["full_sample"]["ATTgt_ntl"]}
        else:
            out_dict = {
                "full_sample": getattr(
                    result_dict["full_sample"]["aggregate_inst"], attr_name
...
def extract_dict_ntl_for_difference(
    result_dict: dict,
    type_of_aggregation: str = None,
    overall: bool = False,
...
def output_dict_to_dataframe(
    result: dict[str, list[namedtuple]],
    stratum: bool = False,
    ...
    for k, ntl in result.items():
        ...
        out_df, name_table, se_info, conf_info = ntl_to_dataframe(
            ntl=ntl, stratum=stratum
        )
        ...
        if k != "full_sample":
            if isinstance(k, tuple):
                k = k[
                    0
                ]  # sample is in the first element of tuple: see extract_dict_ntl
    ...
    output = out_df_index(
        out_df=output, name_table=name_table, se_info=se_info, conf_info=conf_info
    )
    return output
...
def ntl_to_dataframe(ntl: list, name_table: str = None, stratum: bool = False):
...
def fix_std_error_cols(out_df: DataFrame):
    ...
    out_df["lower"] = np.where((out_df["lower"] == 0), np.nan, out_df["lower"])

    out_df["upper"] = np.where((out_df["upper"] == 0), np.nan, out_df["upper"])
